...
tinclude('core.filesystem.helper');
 ...
 * where as the legacy TFile static class treated files in a rather
 * atomic manner.
 *
 * @package     T.Platform
 * @subpackage  FileSystem
...
class TStream extends TObject
{
		...
		$use_prefix = false, $relative = false, $detectprocessingmode = false)
	{
		...
		if (isset($url['scheme']))
		{
			...
			if (TFilesystemHelper::isTStream($url['scheme']))
			{
		...
		{
			$ext = strtolower(TFile::getExt($this->filename));
	...
	public function filesize()
	{
		...
		if (!$res)
		{
			...
			$res = TFilesystemHelper::remotefsize($this->filename);
	...
	public function chmod($filename = '', $mode = 0)
	{
		...
		switch ($sch)
		{
			case 'ftp':
			case 'ftps':
				$res = TFilesystemHelper::ftpChmod($filename, $mode);
				break;
		...
		if ($exists && !$context_support)
		{
			...
			if ($res)
			{
				...
				else
				{
					...
					$php_errormsg = Text::sprintf('JLIB_FILESYSTEM_ERROR_STREAMS_FAILED_TO_OPEN_WRITER', $this->getError());
				}
			}
			else
			{
				if (!$php_errormsg)
				{
					$php_errormsg = Text::sprintf('JLIB_FILESYSTEM_ERROR_STREAMS_FAILED_TO_OPEN_READER', $this->getError());
				}
			}
		}
		else
	...
	public function _getFilename($filename, $mode, $use_prefix, $relative)
	{
		if ($use_prefix)
		{
			...
			if (in_array($tmode, TFilesystemHelper::getWriteModes()))
			{
