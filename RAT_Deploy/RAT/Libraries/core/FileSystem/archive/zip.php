...
/**
 * ZIP format adapter for the TArchive class
 *
 ...
 * @since       5/2014
 */
class TArchiveZip extends TObject
{
	/**
	...
	public function extract($archive, $destination, $options = array())
	{
		...
		{
			return ($this->_extractNative($archive, $destination, $options)) ? true : TError::raiseWarning(100, $this->get('error.message'));
		}
		else
		{
			return ($this->_extract($archive, $destination, $options)) ? true : TError::raiseWarning(100, $this->get('error.message'));
		}
	}
	...
	private function _extract($archive, $destination, $options)
	{
		...
		{
			$this->set('error.message', Text::_('JLIB_FILESYSTEM_ZIP_NOT_SUPPORTED'));
		...
		{
			$this->set('error.message', Text::_('JLIB_FILESYSTEM_ZIP_UNABLE_TO_READ'));
		...
		{
			$this->set('error.message', Text::_('JLIB_FILESYSTEM_ZIP_INFO_FAILED'));
		...
		for ($i = 0, $n = count($this->_metadata); $i < $n; $i++)
		{
			...
			if ($lastPathCharacter !== '/' && $lastPathCharacter !== '\\')
			{
				...
				$path = TPath::clean($destination . '/' . $this->_metadata[$i]['name']);

				// Make sure the destination folder exists
				if (!TFolder::create(dirname($path)))
				{
					$this->set('error.message', Text::_('JLIB_FILESYSTEM_ZIP_UNABLE_TO_CREATE_DESTINATION'));

					return false;
				}

				if (TFile::write($path, $buffer) === false)
				{
					$this->set('error.message', Text::_('JLIB_FILESYSTEM_ZIP_UNABLE_TO_WRITE_ENTRY'));
	...
	private function _extractNative($archive, $destination, $options)
	{
			...
			while ($file = @zip_read($zip))
			{
				if (zip_entry_open($zip, $file, "r"))
				{
					if (substr(zip_entry_name($file), strlen(zip_entry_name($file)) - 1) != "/")
					{
						...
						if (TFile::write($destination . '/' . zip_entry_name($file), $buffer) === false)
						{
				...
				{
					$this->set('error.message', Text::_('JLIB_FILESYSTEM_ZIP_UNABLE_TO_READ_ENTRY'));
		...
		{
			$this->set('error.message', Text::_('JLIB_FILESYSTEM_ZIP_UNABLE_TO_OPEN_ARCHIVE'));
	...
	private function _readZipInfo(&$data)
	{
		...
		do
		{
			...
			{
				$this->set('error.message', Text::_('JLIB_FILESYSTEM_ZIP_INVALID_ZIP_DATA'));
