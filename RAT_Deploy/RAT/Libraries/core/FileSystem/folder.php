...
abstract class TFolder
{
	 ...
	 *
	 * @return  mixed  TError object on failure or boolean True on success.
	 *
	 * @since   5/2014
	 */
	public static function copy($src, $dest, $path = '', $force = false, $use_streams = false)
	{
		...
		if (!self::exists($src))
		{
			return TError::raiseError(-1, Text::_('JLIB_FILESYSTEM_ERROR_FIND_SOURCE_FOLDER'));
		}
		if (self::exists($dest) && !$force)
		{
			return TError::raiseError(-1, Text::_('JLIB_FILESYSTEM_ERROR_FOLDER_EXISTS'));
		}

		// Make sure the destination exists
		if (!self::create($dest))
		{
			return TError::raiseError(-1, Text::_('JLIB_FILESYSTEM_ERROR_FOLDER_CREATE'));
		}
		...
		if ($FTPOptions['enabled'] == 1 && !$use_streams)
		{
			...
			if (!($dh = @opendir($src)))
			{
				return TError::raiseError(-1, Text::_('JLIB_FILESYSTEM_ERROR_FOLDER_OPEN'));
			}
			...
			while (($file = readdir($dh)) !== false)
			{
				...
				switch (filetype($sfid))
				{
					...
					case 'file':
						...
						$dfid = TPath::clean(str_replace(TPATH_ROOT, $FTPOptions['root'], $dfid), '/');
						if (!$ftp->store($sfid, $dfid))
						{
							return TError::raiseError(-1, Text::_('JLIB_FILESYSTEM_ERROR_COPY_FAILED'));
						}
						break;
				}
			}
				...
				switch (filetype($sfid))
				{
					...
					case 'file':
						if ($use_streams)
						{
							...
							{
								return TError::raiseError(-1, Text::_('JLIB_FILESYSTEM_ERROR_COPY_FAILED') . ': ' . $stream->getError());
							}
						}
						else
						{
	...
	public static function create($path = '', $mode = 0755)
	{
		...
		if (!self::exists($parent))
		{
			...
			{
				TError::raiseWarning('SOME_ERROR_CODE', __METHOD__ . ': ' . Text::_('JLIB_FILESYSTEM_ERROR_FOLDER_LOOP'));
				$nested--;
				return false;
			}
			...
			{
				// TFolder::create throws an error
				$nested--;
				return false;
			}
		...
		if ($FTPOptions['enabled'] == 1)
		{
			...
			$path = TPath::clean(str_replace(TPATH_ROOT, $FTPOptions['root'], $path), '/');
			$ret = $ftp->mkdir($path);
			$ftp->chmod($path, $mode);
		}
		else
		{
			...
			if ($obd != null)
			{
				...
				if ($inBaseDir == false)
				{
					// Return false for TFolder::create because the path to be created is not in open_basedir
					TError::raiseWarning('SOME_ERROR_CODE', __METHOD__ . ': ' . Text::_('JLIB_FILESYSTEM_ERROR_FOLDER_PATH'));
					return false;
				}
			}
			...
			if (!$ret = @mkdir($path, $mode))
			{
				@umask($origmask);
				TError::raiseWarning(
					'SOME_ERROR_CODE', __METHOD__ . ': ' . Text::_('JLIB_FILESYSTEM_ERROR_COULD_NOT_CREATE_DIRECTORY').
					' Path: ' . $path
				);
				return false;
			}
	...
	public static function delete($path)
	{
		...
		if (!$path)
		{
			...
			TError::raiseWarning(500, __METHOD__ . ': ' . Text::_('JLIB_FILESYSTEM_ERROR_DELETE_BASE_DIRECTORY'));
			return false;
		}
		...
		if (!is_dir($path))
		{
			TError::raiseWarning(21, Text::sprintf('JLIB_FILESYSTEM_ERROR_PATH_IS_NOT_A_FOLDER', $path));
			return false;
		}
		...
		if (!empty($files))
		{
			...
			if (TFile::delete($files) !== true)
			{
				// TFile::delete throws an error
				return false;
			}
		}
		...
		foreach ($folders as $folder)
		{
			...
			{
				// TFolder::delete throws an error
				return false;
			}
		}
		...
		{
			TError::raiseWarning('SOME_ERROR_CODE', Text::sprintf('JLIB_FILESYSTEM_ERROR_FOLDER_DELETE', $path));
			$ret = false;
		}
		return $ret;
	}
	...
	{
		return is_dir(TPath::clean($path));
	}
		...
		$excludefilter = array('^\..*', '.*~'), $naturalSort = false)
	{
		...
		if (!is_dir($path))
		{
			TError::raiseWarning(21, Text::sprintf('JLIB_FILESYSTEM_ERROR_PATH_IS_NOT_A_FOLDER_FILES', $path));
			return false;
		}
		...
		$excludefilter = array('^\..*'))
	{
		...
		if (!is_dir($path))
		{
			TError::raiseWarning(21, Text::sprintf('JLIB_FILESYSTEM_ERROR_PATH_IS_NOT_A_FOLDER_FOLDER', $path));
			return false;
		}
	...
	public static function listFolderTree($path, $filter, $maxLevel = 3, $level = 0, $parent = 0)
	{
		...
		if ($level == 0)
		{
			$GLOBALS['_TFolder_folder_tree_index'] = 0;
		}
		if ($level < $maxLevel)
		{
			...
			{
				$id = ++$GLOBALS['_TFolder_folder_tree_index'];
				$fullName = TPath::clean($path . '/' . $name);
				$dirs[] = array('id' => $id, 'parent' => $parent, 'name' => $name, 'fullname' => $fullName,
					'relname' => str_replace(TPATH_ROOT, '', $fullName));
