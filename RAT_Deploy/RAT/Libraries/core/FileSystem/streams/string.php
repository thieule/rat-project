...
tinclude('core.filesystem.support.stringcontroller');
...
class TStreamString
{
	...
	{
		$this->_currentstring = &TStringController::getRef(str_replace('string://', '', $path));
	...
	{
		$now = time();
		$string = &TStringController::getRef(str_replace('string://', '', $path));
		$stat = array(
			'dev' => 0,
			'ino' => 0,
			'mode' => 0,
...
stream_wrapper_register('string', 'TStreamString') or die(Text::_('JLIB_FILESYSTEM_STREAM_FAILED'));
