...
{
	/**
	 * Container for already tincluded library paths.
	 *
	 * @var    array
	 * @since  5/2014
	 */
	 ...
	 * @since  5/2014
	 */
	protected static $tincluded = array();
	 ...
	 * @since   5/2014
	 */
	public static function tinclude($key, $base = null)
	{
		// Only import the library if not already attempted.
		if (!isset(self::$tincluded[$key]))
		{
			...
			if (strpos($path, 'core') === 0)
			{
				...
				// Since we are in the T namespace prepend the classname with T.
                            if($class!='T')   
				$class = 'T' . $class;
			...
			*/
			...
			self::$tincluded[$key] = $success;
		}

		return self::$tincluded[$key];
	}
	...
	public static function setup()
	{
		...
		self::registerPrefix('T', T_PLATFORM . '/core');
 ...
 * @since   5/2014
 */
function texit($message = 0)
{
	exit($message);
}

/**
 * Intelligent file include.
 *
 ...
 * @since   5/2014
 */
function tinclude($path)
{
	return TLoader::tinclude($path);
}
