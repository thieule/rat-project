 ...
 * @since       11.1
 */
class TCacheStorageFile extends TCacheStorage
{
	/**
	 * @var    string
	 * @since  11.1
	...
	public function getAll()
	{
		...
		foreach ($folders as $folder)
		{
			...
			$item = new TCacheStorageHelper($folder);
	...
	protected function _deleteFolder($path)
	{
		...
		if (!$path || !is_dir($path) || empty($this->_root))
		{
			...
			TError::raiseWarning(500, 'TCacheStorageFile::_deleteFolder ' . JText::_('JLIB_FILESYSTEM_ERROR_DELETE_BASE_DIRECTORY'));
			return false;
		}

		$path = $this->_cleanPath($path);

		// Check to make sure path is inside cache folder, we do not want to delete T root!
		$pos = strpos($path, $this->_cleanPath($this->_root));

		if ($pos === false || $pos > 0)
		{
			TError::raiseWarning(500, 'TCacheStorageFile::_deleteFolder' . JText::sprintf('JLIB_FILESYSTEM_ERROR_PATH_IS_NOT_A_FOLDER', $path));
			return false;
		}
		...
		elseif (!empty($files) && is_array($files))
		{
			...
			foreach ($files as $file)
			{
				...
				else
				{
					...
					TError::raiseWarning('SOME_ERROR_CODE', 'TCacheStorageFile::_deleteFolder' . JText::sprintf('JLIB_FILESYSTEM_DELETE_FAILED', $filename));
					return false;
				}
			}
		}
		...
		{
			TError::raiseWarning('SOME_ERROR_CODE', 'TCacheStorageFile::_deleteFolder' . JText::sprintf('JLIB_FILESYSTEM_ERROR_FOLDER_DELETE', $path));
			$ret = false;
		}
		return $ret;
	}
		...
		, $exclude = array('.svn', 'CVS', '.DS_Store', '__MACOSX'), $excludefilter = array('^\..*', '.*~'))
	{
		...
		if (!is_dir($path))
		{
			TError::raiseWarning(21, 'TCacheStorageFile::_filesInFolder' . JText::sprintf('JLIB_FILESYSTEM_ERROR_PATH_IS_NOT_A_FOLDER', $path));
			return false;
		}
		...
		, $exclude = array('.svn', 'CVS', '.DS_Store', '__MACOSX'), $excludefilter = array('^\..*'))
	{
		...
		if (!is_dir($path))
		{
			TError::raiseWarning(21, 'TCacheStorageFile::_folders' . JText::sprintf('JLIB_FILESYSTEM_ERROR_PATH_IS_NOT_A_FOLDER', $path));
			return false;
		}
