...
/**
 * TArrayHelper is an array utility class for doing all sorts of odds and ends with arrays.
 *
 * @package     T.Platform
 * @subpackage  Utilities
 * @since       5/2014
...
class TArrayHelper
{
	...
	public static function toInteger(&$array, $default = null)
	{
		...
		else
		{
			...
			{
				TArrayHelper::toInteger($default, null);
				$array = $default;
			}
			else
			{
	...
	public static function toObject(&$array, $class = 'stdClass')
	{
		...
		if (is_array($array))
		{
			...
			foreach ($array as $k => $v)
			{
				if (is_array($v))
				{
					$obj->$k = TArrayHelper::toObject($v, $class);
				}
				else
				{
					$obj->$k = $v;
		...
		if (is_array($array))
		{
			foreach ($array as $key => $item)
			{
				if (is_array($item))
				{
					...
					$output[] = TArrayHelper::toString($item, $inner_glue, $outer_glue, $keepOuterKey);
				}
				else
				{
	...
	protected static function _sortObjects(&$a, &$b)
	{
		...
		for ($i = 0, $count = count($key); $i < $count; $i++)
		{
			...
			{
				$cmp = TString::strcmp($va, $vb, $locale);
			}
			else
			{
				$cmp = TString::strcasecmp($va, $vb, $locale);
			}
