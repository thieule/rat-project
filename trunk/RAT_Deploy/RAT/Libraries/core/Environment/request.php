 ...
 */
$GLOBALS['_TREQUEST'] = array();

/**
 * Set the available masks for cleaning variables
 */
define('TREQUEST_NOTRIM', 1);
define('TREQUEST_ALLOWRAW', 2);
define('TREQUEST_ALLOWHTML', 4);

TLog::add('TRequest is deprecated.', TLog::WARNING, 'deprecated');
 ...
 * @subpackage  Environment
 * @since       5/2014
 * @deprecated  12.1  Get the TInput object from the application instead
 */
class TRequest
{
	 ...
	 * @param   string   $type     Return type for the variable, for valid values see {@link TFilterInput::clean()}.
	 ...
	 * @since   5/2014
	 *
	 * @deprecated   12.1  Use TInput::Get
	 */
	public static function getVar($name, $default = null, $hash = 'default', $type = 'none', $mask = 0)
	{
		...
		if (isset($GLOBALS['_TREQUEST'][$name]['SET.' . $hash]) && ($GLOBALS['_TREQUEST'][$name]['SET.' . $hash] === true))
		{
		...
		elseif (!isset($GLOBALS['_TREQUEST'][$name][$sig]))
		{
			if (isset($input[$name]) && $input[$name] !== null)
			{
				...
				$GLOBALS['_TREQUEST'][$name][$sig] = $var;
			}
			elseif ($default !== null)
			{
		...
		{
			$var = $GLOBALS['_TREQUEST'][$name][$sig];
		}
	...
	public static function setVar($name, $value = null, $hash = 'method', $overwrite = true)
	{
		...
		$GLOBALS['_TREQUEST'][$name]['SET.' . $hash] = true;
		$GLOBALS['_TREQUEST'][$name]['SET.REQUEST'] = true;
	 ...
	 *
	 * Use in conjunction with THtml::_('form.token').
	 *
	 * @param   string  $method  The request method in which to look for the token key.
	 *
	 * @return  boolean  True if found and valid, false otherwise.
	 *
	 * @deprecated  12.1 Use TSession::checkToken() instead.
	 * @since       5/2014
	 */
	public static function checkToken($method = 'post')
	{
		$token = TSession::getFormToken();
		if (!self::getVar($token, '', $method, 'alnum'))
		{
			...
			if ($session->isNew())
			{
				...
				$app = T::getApplication();
				$return = TRoute::_('index.php');
				$app->redirect($return, Text::_('JLIB_ENVIRONMENT_SESSION_EXPIRED'));
				$app->close();
			}
			else
			{
	 ...
	 * @param   string   $type  The variable type {@see TFilterInput::clean()}.
	 *
	...
	static function _cleanVar($var, $mask = 0, $type = null)
	{
		...
		elseif ($mask & 4)
		{
			...
			$safeHtmlFilter = TFilterInput::getInstance(null, null, 1, 1);
		...
		else
		{
			...
			$noHtmlFilter = TFilterInput::getInstance();
	...
	{
		$value = is_array($value) ? array_map(array('TRequest', '_stripSlashesRecursive'), $value) : stripslashes($value);
		return $value;
	}
}
