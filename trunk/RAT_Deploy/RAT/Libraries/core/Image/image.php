...
class TImage
{
	...
	public function __construct($source = null)
	{
		...
		if (!extension_loaded('gd'))
		{
			...
			TLog::add('The GD extension for PHP is not available.', TLog::ERROR);
	...
	public function crop($width, $height, $left, $top, $createNew = true)
	{
		...
		// If we are cropping to a new image, create a new TImage object.
		if ($createNew)
		{
			// @codeCoverageIgnoreStart
			$new = new TImage($handle);
	...
	/**
	 * Method to load a file into the TImage object as the resource.
	 *
	...
	public function loadFile($path)
	{
		...
		switch ($properties->mime)
		{
			case 'image/gif':
				...
				{
					// @codeCoverageIgnoreStart
					TLog::add('Attempting to load an image of unsupported type GIF.', TLog::ERROR);
			...
			case 'image/jpeg':
				...
				{
					// @codeCoverageIgnoreStart
					TLog::add('Attempting to load an image of unsupported type JPG.', TLog::ERROR);
			...
			case 'image/png':
				...
				{
					// @codeCoverageIgnoreStart
					TLog::add('Attempting to load an image of unsupported type PNG.', TLog::ERROR);
			...
			default:
				TLog::add('Attempting to load an image of unsupported type: ' . $properties->mime, TLog::ERROR);
	...
	public function resize($width, $height, $createNew = true, $scaleMethod = self::SCALE_INSIDE)
	{
		...
		// If we are resizing to a new image, create a new TImage object.
		if ($createNew)
		{
			// @codeCoverageIgnoreStart
			$new = new TImage($handle);
	...
	protected function getFilterInstance($type)
	{
		...
		$className = 'TImageFilter' . ucfirst($type);
		if (!class_exists($className))
		{
			TLog::add('The ' . ucfirst($type) . ' image filter is not available.', TLog::ERROR);
		...
		if (!($instance instanceof TImageFilter))
		{
			// @codeCoverageIgnoreStart
			TLog::add('The ' . ucfirst($type) . ' image filter is not valid.', TLog::ERROR);
