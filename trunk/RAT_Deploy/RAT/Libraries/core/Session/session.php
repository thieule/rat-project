...
class TSession extends TObject
{
	...
	/**
	 * @var    TSession  TSession instances container.
	 * @since  5/2014
	 */
	...
	public function __construct($store = 'none', $options = array())
	{
		...
		// Create handler
		$this->_store = TSessionStorage::getInstance($store, $options);
	 ...
	 *
	 * @return  TSession  The Session object.
	 *
	 * @since   5/2014
	 */
	public static function getInstance($handler, $options)
	{
		...
		{
			self::$instance = new TSession($handler, $options);
		}
	...
	public static function getFormToken($forceNew = false)
	{
		...
		$hash = TApplication::getHash($user->get('id', 0) . $session->getToken($forceNew));
	 ...
	 *
	 * Use in conjunction with JHtml::_('form.token') or TSession::getFormToken.
	 *
	...
	public static function checkToken($method = 'post')
	{
		if ($method == 'default')
		{
			trigger_error("TSession::checkToken() doesn't support 'default' for the method parameter.", E_USER_ERROR);
			return false;
		}
		...
		if (!TRequest::getVar($token, '', $method, 'alnum'))
		{
			$session = T::getSession();
			if ($session->isNew())
			{
	...
	public static function getStores()
	{
		...
		foreach ($handlers as $handler)
		{
			...
			$class = 'TSessionStorage' . ucfirst($name);
	...
	protected function _start()
	{
		...
		else
		{
			...
			if (!TRequest::getVar($session_name, false, 'COOKIE'))
			{
				if (TRequest::getVar($session_name))
				{
					session_id(TRequest::getVar($session_name));
	 ...
	 * to call TSession::close(), but as session data is locked to prevent concurrent
